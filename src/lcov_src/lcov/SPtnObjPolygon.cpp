...
//頂点の追加
void SPtnObjPolygon::addGrip(int index,int x,int y)
{
	if(m_nCount==0 || m_pPt==NULL) return;
	...
	pPt[index+1] = SPoint(x,y);
	for(i=index+2,j=index+1;j<m_nCount;i++,j++){
		pPt[i] = m_pPt[j];
	}
	delete[]m_pPt;
	m_pPt = pPt;
	...
	while(rce3.ReadRecord(str)!=WEOF){
		if(str[0] == '-'){
			...
			if(0<n && n<(l-1)){	// : で分けられたレコードである
				...
				if(var==_T("F")){
					fill = _tstoi(str.substr(n+1).c_str()); //:から終端までを数値に変換
				}else
				if(var==_T("N")){
					node = _tstoi(str.substr(n+1).c_str()); //:から終端までを数値に変換
