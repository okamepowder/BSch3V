...
void SXBSchComponent::setPackageName(const char* psz)
{
	m_packagename = psz;
}

void SXBSchComponent::setMfr(const char* psz)
{
	m_mfr = psz;
}

void SXBSchComponent::setMFrPNum(const char* psz)
{
	m_mfrpnum = psz;
}
...
//#endif
				...
				if(var=="PKG"){			//2009/01/06
					m_packagename=val;
				}else
				if(var=="MFR"){			//2009/01/07
					m_mfr=val;
				}else
				if(var=="MFRPN"){		//2009/01/07
					m_mfrpnum=val;
				}
			}
		}
...
//	char sz[32];
	...
	wce3.WriteRecordInt("NH",(m_name_hide ? 1: 0));
	...
	wce3.WriteRecordInt("RH",(m_refnum_hide ? 1: 0));
