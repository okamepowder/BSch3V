...
//文字列の両端のホワイトスペースの削除
//制御コードも削除するように変更 2010/11/07 Version 0.69.01
...
int SNetList::writeUserTypeNetList(const TCHAR* filename, bool bExcludeNC, int( __stdcall *fnWriteNetList)(const TCHAR*, const SExportNetlistInfo*))
{
	...
	//部品情報をまとめる
	//まずは一通り部品情報リストをスキャンして有効な部品情報の数を求める
	int n=m_listComponent.size();
	int nCompInfo = 0;
	for(int i=0;i<n;i++){
		if(    (i>0)
	...
	//ネット情報をまとめる
	int nNetCount = m_listNet.size();	//とりあえずこの数でネット情報数を確保する。NCを削除すると、実際の数はひとつ減るかも
