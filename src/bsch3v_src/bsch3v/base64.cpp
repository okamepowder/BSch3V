...
TCHAR EncodeBase64_char(unsigned data)
{
	data &= 63;
...
void EncodeBase64_block(const byte* srcbuff,int blocksize,TCHAR* dstbuff)
{
...
unsigned DecodeBase64_char(TCHAR c)
{
	if('A'<=c && c<='Z'){
		return (unsigned)c-'A';
	}else if('a'<=c && c<='z'){
...
void DecodeBase64_block(const TCHAR* srcbuff,byte* dstbuff,int* pDataSize)
{
	*pDataSize = 0;
