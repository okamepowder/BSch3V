	...
	while(rce3.ReadRecord(str)!=WEOF){
		if(str[0] == '-'){
			...
			if(0<n && n<(l-1)){	// : で分けられたレコードである
				...
				if(var==_T("X")){
					cx = _tstoi(str.substr(n+1).c_str()); //:から終端までを数値に変換
				}else
				if(var==_T("Y")){
					cy = _tstoi(str.substr(n+1).c_str()); //:から終端までを数値に変換
				}else
				if(var==_T("R")){
					r = _tstoi(str.substr(n+1).c_str()); //:から終端までを数値に変換
				}else
				if(var==_T("B")){
					beginDeg = _tstoi(str.substr(n+1).c_str()); //:から終端までを数値に変換
				}else
				if(var==_T("E")){
					endDeg = _tstoi(str.substr(n+1).c_str()); //:から終端までを数値に変換
				}
			}
		}
	}
...
bool SPtnObjArc::writeCe3(SWriteCE3& wce3,SPoint* pOrigin)
{
	...
	wce3.WriteRecord(_T("+AR"));
	wce3.WriteRecordInt(_T("W"),this->m_width);
	wce3.WriteRecordInt(_T("S"),this->m_style);
	wce3.WriteRecordInt(_T("X"),center.x());
	wce3.WriteRecordInt(_T("Y"),center.y());
	wce3.WriteRecordInt(_T("R"),this->m_nR);
	wce3.WriteRecordInt(_T("B"),this->m_nBegin);
	wce3.WriteRecordInt(_T("E"),this->m_nEnd);
	wce3.WriteRecord(_T("-AR"));
	wce3.WriteEOL();
	return true;
}
