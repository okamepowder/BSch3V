...
bool SPtn::writeCe3(SWriteCE3& wce3,LPCTSTR pszName)
{
	int x = m_size.w();
	int y = m_size.h();
...
SPin::SPin()
{
	...
	m_flags		= 0;			//ファイルに保存されない表示上のフラグ
	m_pinNameDisplayFlag = 0xFFFFFFFF;
	m_pszPinNameDraw = NULL;
}
...
SPin::SPin(const SPin& pin)
{
	...
	if(pin.m_pszPinNameDraw != NULL){
		m_pszPinNameDraw = new TCHAR[_tcslen(pin.m_pszPinNameDraw)+1];
		_tcscpy(m_pszPinNameDraw,pin.m_pszPinNameDraw);
	}else{
		m_pszPinNameDraw = NULL;
	}
}
...
bool SPin::writeCe3(SWriteCE3& wce3)
{
	...
	wstring name=_T("");
	const TCHAR* rawpinname = pinName();
	while(*rawpinname){
		if(*rawpinname & 0x80){
			name += '\\';
		}
...
//表示/非表示に対応した文字列のセットアップ
void SPin::setupPinNameDraw()
{
	if(m_pszName == NULL) return;
	if(m_pszPinNameDraw!=NULL) return;
	
	TCHAR buff[256];
	int s = 0; //ソース文字列インデックス
	int d = 0; //デスティネーション文字列インデックス
	int j = 0; //区切りのカウンタ
	unsigned long dispFlagBit = 0x00000001;
	bool dispFlag = ((m_pinNameDisplayFlag & dispFlagBit)!=0);
	bool detectFirstDispWord = dispFlag;
	while(m_pszName[s]!=_T('\0') && s<255){
		if(j<32 && m_pszName[s] == _T('/')){
			dispFlagBit<<=1;
			j++;
			if(j>=32){
				dispFlag = true;
			}else{
				dispFlag = ((m_pinNameDisplayFlag & dispFlagBit)!=0);
			}
			if(dispFlag){
				if(!detectFirstDispWord){ //最初の表示ワードのときにはスラッシュをコピーしない
					detectFirstDispWord = true;
				}else{
					buff[d++] = _T('/');
				}
			}
	...
	buff[d] = _T('\0');
	m_pszPinNameDraw = new TCHAR[d+1];
	_tcscpy(m_pszPinNameDraw,buff);
...
//製造者を得る		20090203
const TCHAR* SCompIndex::Mfr() const
{
...
//製造者型式を得る		20090203
const TCHAR* SCompIndex::MfrPn() const
{
	if(m_pCompInfo==NULL) return NULL;
...
//パッケージを得る		20090203
const TCHAR* SCompIndex::Pkg() const
{
	if(m_pCompInfo==NULL) return NULL;
...
//ライブラリ名の設定
void SCompLib::setLibraryName(LPCTSTR name)
{
...
//ライブラリ名を得る
LPCTSTR SCompLib::libraryName() const
{
...
bool SCompLib::WriteAlias(SWriteCE3& wce3,LPCTSTR  alias,LPCTSTR  name)
{
