...
void SReadCE3::ParseRecord(const wstring& record, wstring& var, wstring& val)
{
	int n;
	int l=record.length();
	for(n=0;n<l;n++){
	...
	if(0<n && n<l){	// : で分けられたレコードである
		var = record.substr(0,n);			//先頭から:の手前まで
		val = record.substr(n+1).c_str();	//:から終端まで
	}
}
...
int SWriteCE3::WriteRecord(const TCHAR* pBuff)
{
	if(!m_bNewLine){
